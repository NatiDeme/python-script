...
region_geoms = [shape(feature['geometry']) for feature in regions_data['features']]
...
for region_index, region_geom in enumerate(region_geoms):
    ...
    for river_feature in river_features:
        ...
        if not river_inside_region.is_empty:
            filtered_river_feature = {
                ...
                "properties": {
                    **river_feature['properties'],  # Preserve properties
                    "region": region_index  # Add region index to properties
                }
            }
...
with open('testv2.geojson', 'w') as f:
